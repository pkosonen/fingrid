...
def render_page(datasets: list[dict]) -> bytes:
...
<body>
  <main class=\"shell\">
    ...
    <p class=\"lead\">Browse all datasets exposed by the Fingrid Data service. Filter by id, name, organization, type, unit, period, description, or keywords without hitting the API again.</p>
...
class DatasetViewerHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        try:
            datasets = fetch_datasets(self.server.api_key)
            body = render_page(datasets)
            ...
            self.send_error(error.code, f"Fingrid API error: {error.reason}")
        except URLError as error:
            self.send_error(502, f"Unable to reach Fingrid API: {error.reason}")
        except Exception as error:  # pragma: no cover
            self.send_error(500, f"Viewer error: {error}")

    def log_message(self, format: str, *args) -> None:
        return


def build_server(api_key: str) -> ThreadingHTTPServer:
    for offset in range(PORT_ATTEMPTS):
        ...
        try:
            ...
            if error.errno != errno.EADDRINUSE:
                raise
    raise RuntimeError(
        f"No free port found in range {PORT}-{PORT + PORT_ATTEMPTS - 1}."
    )
...
def main() -> int:
    ...
    print(f"Serving Fingrid dataset viewer at http://{HOST}:{active_port}")
    try:
